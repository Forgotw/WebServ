		...
		if (dir != NULL) {
			...
			while ((entry = readdir(dir)) != NULL) {
				if (std::strcmp(entry->d_name, cgiName.c_str()) == 0) {
					closedir(dir);
					std::cout
						<< "Real path for cgi: " << directory + "/" + cgiName
						<< std::endl;
...
std::string CgiHandler::handleCGI(const Location* foundLocation,
	...
	// *uiStatusCode = 200; // TODO: handleCGI a ete refactor, may be on a plus
	// besoin de cette variable, a discuter.
	return response;
}
